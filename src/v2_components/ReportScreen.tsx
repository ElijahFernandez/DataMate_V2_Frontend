...
import ReportIcon from "../images/database-svgrepo-com (6).svg";

import { ReportEntity, User } from "../api/dataTypes";
...
//Importfile
type ReportListProp = {
  setReportId: (num: number) => void;
};
const ReportList: React.FC<ReportListProp> = ({
  setReportId,
}: ReportListProp) => {
  const [reports, setReports] = useState<ReportEntity[]>([]);
  const [filteredReports, setFilteredReports] = useState<ReportEntity[]>([]);
  ...
  const [searchResult, setSearchResult] = useState<ReportEntity[]>([]);
  ...
  const itemsPerRow = Math.min(reports?.length, 3);
  const lgValue = Math.floor(12 / itemsPerRow);
  const xlValue = Math.floor(12 / itemsPerRow);

  const handleClickReportName = (rprt: ReportEntity) => {
    let id = rprt?.reportId;
    console.log("Redirecting to report: ", id);
    nav("/reports/report", {
      state: {
        rprtid: id,
      },
  ...
  const handleOptionSelectPop = (option: string, rprt: ReportEntity | null) => {
    setSelectedMenuOption(option);
    setSelectedReport(rprt);
    setIsOpen(!!rprt);
  };
  ...
  useEffect(() => {
    if (reports && reports.length > 0) {
      console.log("rprt state ", reports);
      setFilteredReports(
        reports.filter((rprt) =>
          rprt.reportName.toLowerCase().includes(searchQuery.toLowerCase())
        )
      );
    }
  }, [reports, searchQuery]);
  ...
  useEffect(() => {
    ...
    if (decryptedUserId) {
      ReportService.getReportsByUser(decryptedUserId)
        .then((res) => {
          console.log(res);
          setReports(res);
          setLoading(false);
  ...
  useEffect(() => {
    setSearchResult(filteredReports);
                    ...
                    >
                      ...
                      >
                        <div style={{ flex: "1" }}>
                          <div
                            key={report.reportId}
                            onClick={() => handleClickReportName(report)}
                          >
                            {report.reportName}
                          </div>
                        </div>
                        ...
                        >
                          <List>
                            <ListItem
                              ...
                              onClick={() => {
                                console.log(
                                  `Details option selected for fileId: ${report.reportId}`
                                );
                                handleOptionSelectPop("Details", report);
                              }}
                            >
                            ...
                            <ListItem
                              ...
                              onClick={() => {
                                console.log(
                                  `Delete option selected for fileId: ${report.reportId}`
                                );
                              }}
                            >
